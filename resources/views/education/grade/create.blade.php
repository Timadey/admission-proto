            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white  overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 ">
                            <p>{{ __("Add grades to examination") }}</p>
                            ...
                            <form class="mx-auto lg:w-[50%]" action="{{ route('grade.store', ['education' => $education->id])}}" method="POST">
                                @csrf
                               @for ($i = 0; $i < 5; $i++)
                                    <div id="subject{{ $i + 1 }}">
                                            ...
                                            <select id="subject_name_{{ $i }}" name="grades[{{ $i }}][subject_name]" class="h-12 border border-gray-300 text-gray-600 text-base rounded-lg block w-full py-2.5 px-4 focus:outline-none">
                                             <option value="">Choose subject</option>
                                                @foreach ($subjects as $subject)
                                                    <option value="{{ $subject->value }}" @if (old('grades.'.$i.'.subject_name') == $subject->value) selected @endif>{{ $subject->name }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="block w-full mt-4 mb-4">
                                            ...
                                            <select id="grade_{{ $i }}" name="grades[{{ $i }}][grade]" class="h-12 border border-gray-300 text-gray-600 text-base rounded-lg block w-full py-2.5 px-4 focus:outline-none">
                                                ...
                                                @foreach ($grades as $grade)
                                                    <option value="{{ $grade->value }}" @if (old('grades.'.$i.'.grade') == $grade->value) selected @endif>{{ $grade->name }}</option>
                                                @endforeach
                                            </select>
                                ...
                                @endfor
                                <x-primary-button type="submit"> Add grade </x-primary-button>
                            </form>
                    </div>
                </div>
            </div>
